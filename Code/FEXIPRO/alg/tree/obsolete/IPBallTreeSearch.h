...
void IPBallTreeTopK(const int k, Matrix2D &userData, Matrix2D &itemData);

void IPBallTreeSearch(const int k, const Matrix1D &query, BallTreeNode &node,
...
//    cout << root.size() << "," << root.isLeafNode() << endl;
//    visit(root);
//    exit(0);
	...
	for (int userID = 0; userID < userData.num_row; userID++) {
		...
		IPBallTreeSearch(k, user, root, topkSetInTheLoop, userData.num_col);
